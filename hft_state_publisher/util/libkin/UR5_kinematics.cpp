...
Transform DH_transform(double alpha, double a, double theta, double d) {
    ...
    matrix[0][0] = c_theta;		matrix[1][0] = c_alpha*s_theta;		matrix[2][0] = s_alpha*s_theta;		point[0] = a;
    matrix[0][1] = -s_theta;	matrix[1][1] = c_alpha*c_theta;		matrix[2][1] = s_alpha*c_theta;		point[1] = -d*s_alpha;
    matrix[0][2] = 0.0;			matrix[1][2] = -s_alpha;			matrix[2][2] = c_alpha;				point[2] = d*c_alpha;
...
Transform UR5_dir_kin(double *theta) {
    double offset[] = {offset1_, offset2_, offset3_, offset4_, offset5_, offset6_};
    double alpha[] = {alpha0, alpha1, alpha2, alpha3, alpha4, alpha5};
    double a[] = {a0_, a1_, a2_, a3_, a4_, a5_};
    double d[] = {d1_, d2_, d3_, d4_, d5_, d6_};
...
int UR5_inv_kin(Transform transform, bool shoulder_positive, bool wrist_positive,
    ...
    double c_alpha = px - d6_*ax;
    double s_alpha = py - d6_*ay;

    double alpha;
    double beta;

    double c_beta = d4_ / hypot(c_alpha,s_alpha);

    if (in_interval(&c_beta,-1.0,1.0)) {
        alpha = atan2(s_alpha,c_alpha);
        beta = acos(c_beta);

        theta_tmp[0] = alpha + (2.0*shoulder_positive -1.0)*beta + PI/2.0 - offset1_;
    ...
    double s1 = sin(theta_tmp[0]+offset1_);
    double c1 = cos(theta_tmp[0]+offset1_);

    c_alpha = (-px*s1 + py*c1 + d4_) / d6_;

    if (in_interval(&c_alpha,-1.0,1.0)) {
        alpha = acos(c_alpha);

        theta_tmp[4] = (2.0*wrist_positive-1.0)*alpha -offset5_;
    ...
    double s5 = sin(theta_tmp[4]+offset5_);

    if (in_interval(&s5,0.0,0.0)) {
        theta_tmp[5] = theta_ref[5];
    } else {
        theta_tmp[5] = atan2((s1*ox-c1*oy)/s5,(-s1*nx+c1*ny)/s5) - offset6_;
    }
    ...
    double c6 = cos(theta_tmp[5]+offset6_);
    double s6 = sin(theta_tmp[5]+offset6_);

    double x = s1*(d5_*(s6*ny+c6*oy)-ay*d6_+py)+c1*(d5_*(s6*nx+c6*ox)-ax*d6_+px);
    double y = d5_*(nz*s6+oz*c6)-az*d6_-d1_+pz;

    c_beta = (a3_*a3_ - a2_*a2_ - x*x - y*y) / (2*a2_*hypot(x,y));

    if (in_interval(&c_beta,-1.0,1.0)) {
        alpha = atan2(-y,-x);
        beta = acos(c_beta);

        theta_tmp[1] = alpha +(2.0*elbow_positive-1.0)*beta - offset2_;
    ...
    double c2 = cos(theta_tmp[1]+offset2_);
    double s2 = sin(theta_tmp[1]+offset2_);

    theta_tmp[2] = atan2(y-a2_*s2,x-a2_*c2) - (theta_tmp[1]+offset2_) - offset3_;
    ...
    double c5 = cos(theta_tmp[4]+offset5_);

    alpha = atan2(c5*(c6*nz-s6*oz)-s5*az,-nz*s6-oz*c6);

    theta_tmp[3] = alpha - (theta_tmp[1]+offset2_) - (theta_tmp[2]+offset3_) - offset4_;
...
void find_theta1(Transform transform, Solution *solution,
    ...
    if (in_interval(&c_beta,-1.0, 1.0)) {
        ...
        theta1 = alpha + beta + PI/2.0 - offset1_;
...
void find_theta2(Transform transform, Solution *solution,
    ...
    double theta1 = solution[0].theta[0]+offset1_;
    double s1 = sin(theta1);
    double c1 = cos(theta1);

    double theta6 = solution[0].theta[5]+offset6_;
    ...
    double c_beta = (a3_*a3_ - a2_*a2_ - x*x - y*y) / (2*a2_*hypot(x,y));
...
void find_theta3(Transform transform, Solution *solution,
    ...
    double theta2 = solution[0].theta[1]+offset2_;
    ...
    double alpha = atan2(y-a2_*s2,x-a2_*c2);

    double theta3;

    theta3 = alpha - theta2 - offset3_;
...
void find_theta4(Transform transform, Solution *solution,
    ...
    double theta5 = solution[0].theta[4]+offset5_;
    ...
    double theta3 = solution[0].theta[2]+offset3_;

    double theta4;

    theta4 = alpha - theta2 - theta3 - offset4_;
...
void find_theta5(Transform transform, Solution *solution,
    ...
    double c_alpha = (-px*s1+py*c1+d4_) / d6_;
...
void find_theta6(Transform transform, Solution *solution,
    ...
    double s5 = sin(solution[0].theta[4]+offset5_);

    double theta6;
    if (in_interval(&s5,0.0,0.0,0.002)) {
        theta6 = theta_ref[5];
    } else {
        theta6 = atan2((s1*ox-c1*oy)/s5,(-s1*nx+c1*ny)/s5) - offset6_;
