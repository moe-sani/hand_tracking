...
DATA_SIZE=len(jason_stream_lables)
lst_q_joints=[zero_pose.orientation for i in range(0,DATA_SIZE)]
lst_p_joints=[zero_pose.position for ii in range(0,DATA_SIZE)]
lst_q_imu=[zero_pose.orientation for j in range(0,DATA_SIZE)]
lst_p_imu=[zero_pose.position for j in range(0,DATA_SIZE)]
...
def save_to_csv(f):
    ...
    while not rospy.is_shutdown():
        try:
            ...
            f.write(return_str(return_lst(lst_p_imu)))
            # print(lst_q_imu)
            # print(return_str(return_lst(lst_p_joints)))
            # f.write(return_str(return_lst(lst_q_joints)))
            f.write(return_str(return_lst(lst_p_joints)))
...
def main():
    ...
    q_imu_lables = creat_lables(jason_stream_lables,['.x','.y','.z'])
    f.write(return_str(q_imu_lables))

    # joints_lables = creat_lables(sensor_f_list,['.x','.y','.z','.w'])
    joints_lables = creat_lables(sensor_f_list,['.x','.y','.z'])
